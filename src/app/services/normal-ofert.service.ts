...
export class NormalOfertService {
  ...
  constructor(private userService: UserService, private afs: AngularFirestore, private authService: AuthService, private router: Router) {
  ...
  updateOfertToAcepted(ofert, _snackBar) {
    ...
    this.authService.isAuth().subscribe(user => {
      ...
      this.afs.firestore.runTransaction(t =>
        t.get(ofertDoc.ref).then(of => {
          ...
          this.router.navigateByUrl(`/app/exchange/e/(exchange:transaction/${ofert.ownerUID}/${ofert.id})`);
  ...
  getOneOfert(id, uid) {
    //validar que el user tenga permisos---
